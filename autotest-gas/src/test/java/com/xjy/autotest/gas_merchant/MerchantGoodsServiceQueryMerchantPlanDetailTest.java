 ...
 */
public class MerchantGoodsServiceQueryMerchantPlanDetailTest extends SpringBootTestBase {
     ...
     * 1001
     */
    @AutoTest(file = "gas_merchant/merchantGoodsServiceQueryMerchantPlanDetailTestSuccess.csv")
    @DisplayName("查询商家的商品价格计划详情--成功用例")
    public void merchantGoodsServiceQueryMerchantPlanDetailTestSuccess(
            // 基本参数
            int testId,
            Status status,
            String code,
            ...
            String stationPlanId, String stationPlanId2,
            String stationPlanId3, String merchantPlanId,
...
//        Date effectTimexx = new Date(1576050193499L);
        ...
        if (testId <= 1002) {//已删除的油价
            ...
            gasMerchantTestBase.insertGasStationGoodsPricePlan(0L, stationPlanId2, merchantPlanId, "商家油价计划0",
                    platPartnerId, platPartnerId, stationId, effectTime, "Y", null, null, null, null,
                    goodsInfo);
        }

        if (testId >= 1002) {
            gasMerchantTestBase.insertGasStationGoodsPricePlan(0L, stationPlanId3, merchantPlanId, "商家油价计划0",
                    platPartnerId, platPartnerId, stationId1, effectTime, "Y", null, null, null, null,
                    goodsInfo);
        }
        // 测试过程
        order.setPlanId(merchantPlanId);
        // 调用接口
        BizResult<MerchantGoodsPricePlanInfo> result =
                merchantGoodsService.queryMerchantPlanDetail(order);
        // 结果验证
        print(result);
        assertEquals(status, result.getStatus());
//        assertEquals(code, result.getCode());
        // 数据验证
        MerchantGoodsPricePlanInfo info = result.getInfo();
...
//                stationInfoAssert(info.getStationInfos().get(0), stationId, stationCode, stationName);
            ...
            assertEquals(1, info.getStationInfos().size());
     ...
     * 1001
     */
    @AutoTest(file = "gas_merchant/merchantGoodsServiceQueryMerchantPlanDetailTestFailOne.csv")
    @DisplayName("查询商家的商品价格计划详情--参数非法")
    public void merchantGoodsServiceQueryMerchantPlanDetailTestFailOne(
            // 基本参数
            int testId,
            Status status,
            String code,
